...
function evaluateStrategy(
  strategy: FirebaseFirestore.QueryDocumentSnapshot<FirebaseFirestore.DocumentData>,
  context: { price: Record<string, number>; indicators: Record<string, number> },
) {
  ...
  const results = conditions.map((condition) => evaluateCondition(condition, context, previous));
  if (logic === "ANY") {
...
app.post("/api/candles", async (req, res) => {
  ...
  for (const strategyDoc of strategies.docs) {
    ...
    await db.collection("alerts").add({
      ...
      strategyName: strategyDoc.get("name"),
    ...
    const message = `🚨 Estratégia *${strategyDoc.get("name")}* (${symbol}/${timeframe}) disparou a ${candle.close.toFixed(
      5,
    )}`;
  ...
  res.status(202).json({
    status: "accepted",
    processedStrategies: triggeredCount,
    timestamp: candle.timestamp,
  });
});
